...
public class ReservationFixture {
    ...
    public static Reservation createByDate(LocalDate date) {
        return Reservation.createWithId(idSequence.getAndIncrement(), "user", date, false,
                ReservationTimeFixture.create(LocalTime.now()),
                ThemeFixture.createDefault());
    }

    public static Reservation createCancelled() {
        return Reservation.createWithId(idSequence.getAndIncrement(), "user", LocalDate.now().plusDays(1), true,
                ReservationTimeFixture.create(LocalTime.now()),
                ThemeFixture.createDefault());
    }

    public static Reservation createByTime(ReservationTime reservationTime) {
        return Reservation.createWithId(idSequence.getAndIncrement(), "user", LocalDate.now().plusDays(1), false,
                reservationTime,
                ThemeFixture.createDefault());
    }
}
